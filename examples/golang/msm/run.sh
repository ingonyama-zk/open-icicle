...
if [ "$DEVICE_TYPE" != "CPU" ] && [ ! -d "${ICICLE_BACKEND_INSTALL_DIR}" ] && [ -d "${ICICLE_BACKEND_SOURCE_DIR}" ]; then
  ...
  cmake --fresh -DCMAKE_BUILD_TYPE=Release -DCURVE=bls12_381 -DECNTT=OFF "-D${DEVICE_TYPE}_BACKEND"=local -S "${ICICLE_DIR}" -B "${BUILD_DIR}"
  cmake --build "${BUILD_DIR}" -j
  ...
  cmake --build "${BUILD_DIR}" -j
  cmake --fresh -DCMAKE_BUILD_TYPE=Release -DCURVE=bls12_381 -S "${ICICLE_DIR}" -B "${BUILD_DIR}"
  cmake --build "${BUILD_DIR}" -j
fi
