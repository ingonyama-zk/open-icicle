...
:::important
This is the **open-icicle** distribution. Only the **BN254** and **BLS12-381** curves are supported. Prime fields (babybear, stark252, m31, koalabear) and other curves (bls12_377, bw6_761, grumpkin) are **not** included.
:::
...
3. **Build the project:**
   ...
   cmake --build build -j
   ```
   This is building the [libicicle_device](start/architecture/libraries.md#icicle-device) and the [libicicle_curve_bn254](/start/architecture/libraries.md#icicle-core) frontend lib that corresponds to the selected curve.
...
To disable features:
```bash
icicle-bls12-377 = { git = "https://github.com/ingonyama-zk/open-icicle.git", features = ["no_g2"] }
```
