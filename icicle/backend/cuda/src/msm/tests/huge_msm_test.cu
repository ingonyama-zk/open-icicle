...
int main(int argc, char** argv)
{
  ...
  batch_size = 1;
  points_on_device = false;
  scalars_on_device = false;
  if (nof_chunks == 0 || user_c == 0) {
    printf("this test doesn't work with default c or nof_chunks\n");
    return 0;
  }

  printf(
    "running msm curve=%d, size=%d, batch_size=%d, precomp_factor=%d, c=%d, nof_chunks=%d, scalars_on_device=%d, "
    "points_on_device=%d, same_points=%d\n",
  ...
  // nof_chunks = nof_chunks? nof_chunks : msm::get_optimal_nof_chunks<test_scalar, test_affine,
  // test_projective>(config,msm_size, user_c? user_c : msm::get_optimal_c(msm_size), test_scalar::NBITS); nof_chunks =
  // config.
  ...
  if (precomp_factor > 1) {
    msm::cuda_precompute_msm_points<test_scalar, test_affine, test_projective>(
      points_h, msm_size, config, points_precomputed_h);
    cudaStreamSynchronize(stream);
  }

  // warm up
  msm::msm_cuda<test_scalar, test_affine, test_projective>(
    scalars_h, precomp_factor > 1 ? points_precomputed_h : points_h, msm_size, config, res_d);
  ...
  // chunked_msm(scalars_on_device? scalars_d : scalars_h, precomp_factor > 1 ? (points_on_device? precomp_points_d :
  // points_precomputed_h) : (points_on_device? points_d : points_h), msm_size, nof_chunks, config, res_d);
  msm::msm_cuda<test_scalar, test_affine, test_projective>(
    scalars_h, precomp_factor > 1 ? points_precomputed_h : points_h, msm_size, config, res_d);
  // }
  // msm::msm<test_scalar, test_affine, test_projective>(scalars_on_device? scalars_d : scalars_h, precomp_factor > 1 ?
  // (points_on_device? precomp_points_d : points_precomputed_h) : (points_on_device? points_d : points_h), msm_size,
  // config, res_d, &buckets_d); config.points_size = config.points_size/2; config.return_buckets = true;
  ...
  // msm::msm<test_scalar, test_affine, test_projective>(
  //   scalars + msm_size/2, precomp_factor > 1 ? precomp_points_d: points_d + msm_size/2, msm_size/2, config, res_d,
  //   &buckets_d);
  // cudaDeviceSynchronize();
  // std::cout << "cuda err: " << cudaGetErrorString(cudaGetLastError()) << std::endl;
  ...
  for (int i = 0; i < nof_chunks; i++) {
    ...
    msm::msm_cuda<test_scalar, test_affine, test_projective>(
      scalars_h + chunk_size * i,
      precomp_factor > 1 ? points_precomputed_h + i * chunk_size * precomp_factor : points_h + chunk_size * i,
      sub_msm_size, config, ref_d + i);
  }
  ...
  config.precompute_factor = 1;
  msm::msm_cuda<test_scalar, test_affine, test_projective>(temp_scalars_h, temp_results_h, nof_chunks, config, ref_d);
