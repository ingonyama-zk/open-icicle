...
template <typename E>
__global__ void transpose_kernel(const E* in, E* out, uint32_t row_size, uint32_t column_size, uint64_t batch_size)
{
  uint64_t tid = blockDim.x * blockIdx.x + threadIdx.x;
  ...
  // printf("out %d\n", in[tid].limbs_storage.limbs[4]);
}

template <typename E>
__global__ void
transpose_columns_batch_kernel(const E* in, E* out, uint32_t row_size, uint32_t column_size, uint64_t batch_size)
{
  uint64_t tid = blockDim.x * blockIdx.x + threadIdx.x;
  if (tid >= row_size * column_size * batch_size) return;
  uint64_t vec_num = tid % batch_size;
  uint64_t vec_tid = tid / batch_size;
  out[vec_num + ((vec_tid % row_size) * column_size + (vec_tid / row_size)) * batch_size] =
    in[tid]; // coalesced read and write
}

template <typename E>
cudaError_t transpose(const E* vec_a, uint32_t row_size, uint32_t column_size, const VecOpsConfig& config, E* result)
{
  CHK_INIT_IF_RETURN();
  ...
  if (config.columns_batch)
    transpose_columns_batch_kernel<<<num_blocks, num_threads, 0, cuda_stream>>>(
...
eIcicleError matrix_transpose_cuda(
  const Device& device, const E* in, uint32_t nof_rows, uint32_t nof_cols, const VecOpsConfig& config, E* out)
{
  const bool is_inplace = in == out;
  ICICLE_ASSERT(!is_inplace) << "(CUDA) matrix-transpose-inplace not implemented";
  cudaError_t err = transpose<E>(in, nof_cols, nof_rows, config, out);
  return translateCudaError(err);
}
