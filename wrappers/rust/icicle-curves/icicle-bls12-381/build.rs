...
fn link_icicle_hash_if_available(lib_dir: &Path) {
    let shared_lib = lib_dir.join("libicicle_hash.so");
    let static_lib = lib_dir.join("libicicle_hash.a");
...
fn main() {
    ...
    if let Ok(path) = icicle_frontend_install_dir {
        ...
        link_icicle_hash_if_available(Path::new(&path));
        ...
        let lib_dir = icicle_install_dir.join("lib");
        println!("cargo:rustc-link-search={}", lib_dir.display());
        println!("cargo:rustc-link-lib=icicle_field_bls12_381");
        println!("cargo:rustc-link-lib=icicle_curve_bls12_381");
        link_icicle_hash_if_available(&lib_dir);
        println!("cargo:rustc-link-arg=-Wl,-rpath,{}", lib_dir.display()); // Add RPATH linker arguments
